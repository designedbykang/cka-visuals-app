...
function LegalContent({ type }) {
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          height: '28px',
          borderRadius: '50%',
          border: '2px solid rgba(110,1,240,0.2)',
  ...
  let parsed = document.content
  if (typeof parsed === 'string') {
    try { parsed = JSON.parse(parsed) } catch { parsed = null }
  }

  // Format last updated date
  const lastUpdated = document.last_updated
    ? new Date(document.last_updated).toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
      })
